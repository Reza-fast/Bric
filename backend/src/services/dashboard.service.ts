...
export class DashboardService {
  ...
  async getDashboard(referenceDate = new Date()): Promise<DashboardPayload> {
    ...
    const [activeProjects, totalHoursThisWeek, pendingReportsActionRequired, feed, projectHours, weekTasks] =
      await Promise.all([
        ...
        this.timeLogs.sumHoursInRange(weekStart, weekEnd),
        this.reports.countByStatus(ReportStatus.ActionRequired),
        this.activities.listRecent(15),
        this.projects.listHoursSummaries(),
        this.plannedTasks.listInRange(weekStart, weekEnd),
      ]);
